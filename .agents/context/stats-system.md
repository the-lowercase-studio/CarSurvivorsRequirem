...
- Supporting Unity serialization for float and byte stat variants.
...
- Primary code locations:
  ...
  - `Assets/Scripts/Stats/ByteUpgradeableStat.cs`
...
- Core components:
  - `IUpgradeableStat` exposes `CanBeUpgraded`, `IsSubstractModeOn`, `StatsUnits Unit`, upgrade calculation methods, `Upgrade(float)`, and `OnUpgrade`.
  - `UpgradeableStat<T>` stores a typed `Value`, min/max range, upgrade-value range, unit, subtract mode, and upgrade availability.
  - `FloatUpgradeableStat` and `ByteUpgradeableStat` provide non-generic serializable Unity variants.
...
- Runtime flow:
  ...
  - `Upgrade` applies delta, clamps to max boundary, updates `Value`, flips `CanBeUpgraded` off when max is reached, and raises `OnUpgrade`.
...
- Critical behavior rules:
  ...
  - `CanBeUpgraded` becomes false when the value reaches the max boundary.
  - Subtract-mode stats invert the delta and use the configured max boundary as the stopping target.
  - Units are presentation hints; gameplay code should use the stat value itself.
- Ordering or sequencing guarantees:
  - Unity deserialization calls `OnAfterDeserialize`; derived stat types then assign concrete serialized ranges after base processing.
...
- Safe extension areas:
  ...
  - Add a new stat value type by deriving from `UpgradeableStat<T>` when Unity serialization requires a concrete non-generic wrapper.
  - Add a new display unit in `StatsUnits` only when all UI formatting paths are reviewed.
- Required dependencies and contracts:
  - Stat value type `T` must be a struct implementing `IComparable<T>` and `IConvertible`.
...
- Testing implications:
  ...
  - In Unity, validate upgrade button text, value changes, maxed-stat removal, subtract-mode behavior, and runtime reset on scene/domain reload.
...
- Known limitations:
  ...
  - `FloatUpgradeableStat` and `ByteUpgradeableStat` constructors include a `maxValue` parameter that is not used directly.
  - `UpgradeableStat<T>.OnAfterDeserialize` stores `valueFloat` but does not use it.
  - Base `OnAfterDeserialize` checks `MinMaxRange` before derived classes assign serialized concrete ranges, so derived range assignment order should be reviewed before changing deserialization behavior.
...
- Suggested follow-up tasks:
  - Add focused tests for clamp behavior, subtract mode, byte conversion, and upgrade event firing.
  - Review deserialization ordering before adding more concrete stat types.
