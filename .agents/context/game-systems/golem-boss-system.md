...
- Core components:
  ...
  - GolemPursuitState: Default navigation state. Directs GolemMovementController to pursue PlayerPosition, updates walking animation speed, evaluates foot stomp triggers when player is within StompRadius, interrupts into Leap Slam if player distance exceeds LeapTriggerMaxDistance (anti-kiting), and transitions into arm attacks when Arms.AreBothArmsDocked according to cooldown priority (Sky Barrage -> Linear Fist -> Leap Slam).
  - GolemLeapSlamState: Anti-kiting AOE attack state. Halts boss movement, triggers leap animation, spawns a circular ground telegraph at the player's grid position, animates a high parabolic leap arc via DOTween (LeapMaxHeight, LeapAirTime), plays GolemSlam SFX on impact, deals AOE damage via Physics.OverlapSphere, applies phase cooldown multiplier, and transitions back to pursuit.
  ...
  - GolemMovementController: Implements IGolemMovementController. Drives a Rigidbody with frozen rotation, computes smooth sliding directions around impassable terrain obstacles using Physics.SphereCast and Vector3.ProjectOnPlane, and sets desired velocity and rotation toward target positions.
  ...
  - GolemAnimator: Bridges Mecanim Animator triggers (LeapSlam, Stomp, LinearFist, SkyBarrage) and parameters (IsMoving, Speed) via cached string hashes.
...
- Key interfaces:
  ...
  - IGolemMovementController: Controls navigation enable/disable, movement targeting, and obstacle sliding.
  - IGolemArmSocketController: Controls arm projectiles, docking states, and socket initialization.
  - IGolemAnimator: Controls animation trigger and parameter updates.
...
- Runtime flow:
  ...
  - Pursuit & Attacks: GolemPursuitState tracks the player with GolemMovementController. When cooldowns elapse, attacks are selected:
    - Stomp: Checked continuously during pursuit and sky barrage whenever the player enters StompRadius.
    - Anti-Kiting Leap: Triggered immediately if the player is beyond LeapTriggerMaxDistance.
    - Linear Fists: Both arms detach and thrust forward along a rectangular telegraph line, then retract and dock.
    - Sky Barrage: Both arms launch skyward, performing staggered multi-cycle bombardments on circular ground telegraphs around the player while the boss body continues pursuit, then dock upon finishing.
...
- Critical behavior rules:
  ...
  - Body mobility during Sky Barrage: The boss body remains active, continuing pursuit and executing foot stomps while arms are in flight.
