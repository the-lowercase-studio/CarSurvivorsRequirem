...
- Core components:
  ...
  - GolemLinearFistState: Horizontal rocket punch state. Stops movement, aligns rotation to player direction, displays a rectangular ground telegraph, activates GolemLinearAttackHitbox along the rectangular telegraph lane, and fires both detached GolemArmProjectile visual entities along the ground via DOTween. Retracts arms to sockets, deactivates the linear attack hitbox, and transitions back to pursuit upon docking.
  - GolemLinearAttackHitbox: Dedicated indicator-synchronized attack wavefront trigger collider (BoxCollider trigger with kinematic Rigidbody) implementing IGolemLinearAttackHitbox. Dynamically sizes BoxCollider to LinearFistWidth, LinearFistHitboxHeight, and LinearFistHitboxDepth, translates forward synchronously with flying arms via DOTween, deduplicates player hits per thrust pass via reusable HashSet cache and zero-alloc overlap checks, and deactivates immediately on reaching maximum range or state exit.
  - GolemSkyBarrageState: Detached mortar barrage state. Sequentially launches both rocket arms into the sky (LaunchToSky) with SkyArmInitialStaggerDelay. Movement is fully halted with kinematic grounding during the initial launch phase; once arms are launched into flight, the body resumes pursuing the player and executes localized foot stomps when within StompRadius without interrupting active aerial sequences. In the sky, each arm independently performs multiple target drops (total cycles determined by current phase) on randomized offset positions around the player, telegraphed by circular indicators. Upon cycle completion, arms return to sockets (ReturnAndDock) and the boss resumes standard pursuit.
  - GolemDeathState: Terminal state triggered by Health.OnNoHealth. Halts movement and animations, dismisses active telegraphs, docks all arms, deactivates hitboxes, plays GolemDeath SFX, plays death VFX, spawns EXP reward particles, and triggers OnBossDefeated.
  ...
  - GolemBossConfigSO: Designer-authored ScriptableObject containing all combat attributes, health percentages, phase multipliers, attack cooldowns, telegraph warning durations, projectile speeds, linear fist hitbox dimensions, and enrage colors.
...
- Runtime flow:
  ...
  - Pursuit & Attacks: GolemPursuitState tracks the player with GolemMovementController strictly while walking animations play. When cooldowns elapse, attacks are selected:
    ...
    - Linear Fists: Both arms detach and thrust forward along a rectangular telegraph lane while the dedicated GolemLinearAttackHitbox sweeps the corridor dealing single-pass damage to players inside the telegraph, then retract and dock.
    - Sky Barrage: Both arms launch skyward while the body holds still during the launch phase, then the body resumes pursuit and executes localized melee stomps while arms execute staggered multi-cycle bombardments on circular ground telegraphs around the player before returning to dock.
...
- Critical behavior rules:
  ...
  - Animation movement restriction & kinematic grounding: Non-walking animations and attack stances (Stomp, Linear Fist charge, Sky Barrage launch, Leap Slam jump, Death) lock boss movement in place using SetKinematic(true) to eliminate physics popping/depenetration. FixedUpdate navigation only drives movement when walking and CanMove is true.
